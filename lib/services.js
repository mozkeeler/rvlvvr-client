...
exports.decrypt = function (data, localSocket) {
  ...
  encryption.decrypt(data, function (err, stdout) {
    if (err) {
      localSocket.emit('err', {
        error: err.toString(),
        details: 'This may be because you have a password set on your GPG key ' +
                 'and decryption cannot happen until the password has been ' +
                 'entered in the command line. ' +
                 'If you want to have your password remembered, follow the ' +
                 'instructions on the <a target="_blank" ' +
                 'href="https://github.com/rvlvvr/rvlvvr-client/blob/master/README.md">readme</a>.'
      });
      return;
    }
  ...
  if (!messageObj.public) {
    encryption.encrypt(messageObj, message, function (err, msg) {
      if (err) {
        localSocket.emit('err', {
          error: err.toString(),
          details: 'This is likely because you have `alwaysTrust` set to false ' +
                   'in local.json. You can either set this to true and restart ' +
                   'the server or you can manually trust the key in the command ' +
                   'line using gpg2. Another reason is that the person you are ' +
                   'sending to has multiple keys and you are sending to their ' +
                   'one key but they encrypting with a different key for incoming ' +
                   'messages. View the ' +
                   '<a href="https://github.com/rvlvvr/rvlvvr-client/blob/master/README.md">readme</a> ' +
                   'for more details.'
        });
        return;
      }
